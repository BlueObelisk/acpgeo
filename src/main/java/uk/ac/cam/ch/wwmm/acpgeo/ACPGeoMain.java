...
public class ACPGeoMain {
	...
	public static void main (String args[]) throws ValidityException, ParsingException, IOException{
		...
		else runACPGeo(args[0]);
	}

	private static void runACPGeo(String directoryName) throws ValidityException, ParsingException, IOException {
	  ...
	  for (File file : files) {
		...
		POSContainer posContainer = posTagger.runTaggers(abReader.getAbstractString());

		SentenceParser sentenceParser = new SentenceParser(posContainer);
		sentenceParser.parseTags();
		Utils.writeXMLToFile(sentenceParser.getDocument(), "target/"+file.getName());
	}	
	}
